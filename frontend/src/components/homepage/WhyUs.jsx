...
import { FaLeaf, FaSeedling, FaHandHoldingHeart, FaShoppingCart, FaUsers } from 'react-icons/fa'; // Import new icons
...
const WhyUs = ({ height, backgroundColor, backgroundImage }) => {
  ...
  const accordionItems = [
    ...
    },
    {
      ...
      content: 'Ani2Home offers the convenience of browsing a wide variety of products and learning about the farmers who grow them, all from the comfort of your home. The app also provides market information and pricing transparency, ensuring that you make informed and fair purchases.',
    },
    {
      icon: <FaUsers className="text-teal-500 mr-4 text-2xl" />, 
      title: 'Community Engagement and Social Responsibility',
      content: 'Ani2Home fosters a sense of community by enabling customers to engage in meaningful ways, such as participating in reward programs that contribute to social and environmental causes.',
    },
  ];

  return (
    <div style={style} className="flex flex-col items-center justify-center p-6 md:p-10 relative">
        ...
        {accordionItems.map((item, index) => (
          <div key={index} className="border-b w-full">
            <button
              ...
              style={{ backgroundColor: activeIndex === index ? '#67B045' : '#EFEFEF', 
                      transition: 'background-color 0.3s ease' }} 
              onClick={() => handleToggle(index)}
            >
              <div className="flex items-center">
                {item.icon}
                <span className="font-semibold text-lg md:text-xl text-gray-800">{item.title}</span> 
              </div>
